...
public final class BabyEquipmentLogic {
    ...
    public static void tickEntity(BabyNPCPlayerEntity baby) {
        ...
        // First consume the Baby Combat Totem when one exists, then a vanilla
        // Totem of Undying, then select the first Mainhand role by a fixed
        // priority (Sword > Axe > Pickaxe > Shovel > Hoe), so combat protection
        // is never displaced by an ordinary totem.
        for (int pass = 0; pass < 3; pass++) {
            ...
            for (int i = 0; i < baby.getBabyInventory().getContainerSize(); i++) {
                ...
                if (pass == 0 && stored.getItem() instanceof TotemOfBabyCombatItem) { bestSlot = i; break; }
                if (pass == 1 && stored.is(Items.TOTEM_OF_UNDYING)) { bestSlot = i; break; }
                if (pass == 2 && isMainhandCandidate(stored)) {
    ...
    private static boolean isBetterChestFlightItem(ItemStack candidate, ItemStack current) {
        if (candidate.getItem() instanceof net.minecraft.world.item.ElytraItem) {
            return !(current.getItem() instanceof net.minecraft.world.item.ElytraItem);
        }
        return isBetterArmor(candidate, current);
    }
    ...
    public static boolean tryAutoEquip(BabyNPCPlayerEntity baby, ItemStack stack) {
        ...
        if (stack.getItem() instanceof net.minecraft.world.item.ElytraItem) {
            ...
            if (!isManual(baby, slot) && (baby.getItemBySlot(slot).isEmpty() || isBetterChestFlightItem(stack, baby.getItemBySlot(slot)))) {
        ...
        if ((stack.is(Items.TOTEM_OF_UNDYING) || stack.getItem() instanceof TotemOfBabyCombatItem) && !isManual(baby, EquipmentSlot.OFFHAND)) {
            ItemStack currentOffhand = baby.getItemBySlot(EquipmentSlot.OFFHAND).copy();
            if (currentOffhand.isEmpty() || (!currentOffhand.is(Items.TOTEM_OF_UNDYING) && !(currentOffhand.getItem() instanceof TotemOfBabyCombatItem))) {
    ...
    private static boolean tryAutoEquipSerialized(CompoundTag data, ItemStack stack) {
        ...
        if (stack.getItem() instanceof net.minecraft.world.item.ElytraItem) {
            ...
            if (!isSerializedManual(data, slot) && (current.isEmpty() || isBetterChestFlightItem(stack, current))) {
        ...
        if ((stack.is(Items.TOTEM_OF_UNDYING) || stack.getItem() instanceof TotemOfBabyCombatItem) && !isSerializedManual(data, EquipmentSlot.OFFHAND)) {
            ItemStack current = read(eq, "OffHand");
            if (current.isEmpty() || (!current.is(Items.TOTEM_OF_UNDYING) && !(current.getItem() instanceof TotemOfBabyCombatItem))) {
                if (!current.isEmpty() && !addSerializedInventoryItem(data, current)) return false;
