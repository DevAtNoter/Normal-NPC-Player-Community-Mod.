...
import net.devatnoter.normalnpcplayer.event.NPCPlayerChatEvent;
...
public class AdultPlayerMobEntity extends PathfinderMob implements RangedAttackMob {
    ...
    public static final String TOTAL_XP_TAG = "PlayerMobTotalExperience";
    public static final String EXPERIENCE_LEVEL_TAG = "PlayerMobExperienceLevel";
    ...
    /**
     * Real Minecraft usernames used as the Adult Player Mob profile pool.
     *
     * Minecraft/Authlib does not expose a "random player profile" API. The
     * client resolves these names through Minecraft's normal profile lookup
     * path, so the pool can contain many real usernames without hard-coding a
     * UUID for every entry. Known UUIDs are still supplied by knownProfileUuid().
     */
    private static final String[] RANDOM_PROFILE_NAMES = {
            ...
            // MCTiers — Additional Overall / SMP / NethOP
            "visiie",
            "xUltimate_",
            "Gnomity",
            "Ibraaaaaaaa",
            ...
            // Thai Minecraft Creators / Competitors

            // MrBeast Gaming — Thai team
            "Puriwat",
            "Babidear",
            "DIVASAISAI",
            "TJZkillerMC",
            ...
            // Thai Minecraft community / creators
            "AmoryCraft",
            "Siamcraft663",
            "singtopza",
            "Developcraft123",
            ...
            // Thai Minecraft YouTube creators
            "12TimeWeCCG",
            "GeNg PCG",
            "VVFwave",
            "ThreeFive",
            ...
            // Thai Minecraft — Story / Roleplay / Lore
            ...
            // Thai Minecraft Story / Horror / Creator
            "Pengvin",
            "AllenTH999",
            "fushijustwant",

            // Thai Minecraft Creators
            "tackle4826",
            "072Striker",
    ...
    /** Vanilla Java Player dimensions, routed through the Adult scale source of truth. */
    @Override
    public EntityDimensions getDimensions(Pose pose) {
        return AdultPlayerEntityScale.dimensions(pose);
    }
    ...
    public void onAddedToWorld() {
        ...
        // Rebuild the bounding box from the Adult scale source of truth.
        // This keeps hitbox and visual scale synchronized.
        refreshDimensions();
        applyTraitSpeed();
    }
    ...
    /**
     * Use the exact vanilla LivingEntity swing path used by PlayerMob.
     *
     * This intentionally uses the one-argument overload, exactly like the
     * reference PlayerMob implementation. Do not replace this with a custom
     * synced counter or a custom arm animation.
     */
    ...
    /**
     * Central NPC speech service. NPC speech uses Minecraft's disguised-chat
     * path: it is rendered using the normal CHAT decoration (name + message),
     * but it is NOT a signed player message. This is the vanilla path intended
     * for server-generated/profileless chat and avoids PlayerChatSession
     * validation entirely.
     */
    public boolean chatTo(ServerPlayer player, String message) {
        if (player == null || !player.isAlive()) return false;

        NPCPlayerChatEvent event = new NPCPlayerChatEvent(this, message);
    ...
    /** Sends NPC speech to nearby players through the same safe chat service. */
    public boolean chatNearby(String message, double radius) {
        boolean sent = false;
        double radiusSq = radius * radius;
        for (net.minecraft.world.entity.player.Player player : level().players()) {
            if (player instanceof ServerPlayer serverPlayer
                    && serverPlayer.isAlive()
                    && serverPlayer.distanceToSqr(this) <= radiusSq) {
                sent |= chatTo(serverPlayer, message);
            }
        }
        return sent;
    }
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        this.hunterTargetName = tag.contains(HUNTER_TARGET_TAG)
                ...
                : "";
        this.totalExperience = Math.max(0, tag.contains(TOTAL_XP_TAG) ? tag.getInt(TOTAL_XP_TAG) : 0);
        this.experienceLevel = Math.max(0, tag.contains(EXPERIENCE_LEVEL_TAG) ? tag.getInt(EXPERIENCE_LEVEL_TAG) : 0);
    ...
    /**
     * Runs the normal LivingEntity/Mob/PathfinderMob tick chain plus the
     * AdultPlayerMobEntity common per-tick setup, but deliberately does not
     * run the Adult AI brain. Subclasses with their own state machines can
     * call this without accidentally inheriting the Adult brain.
     */
    protected final void tickAdultBase() {
        super.tick();
        applyTraitSpeed();
    }
    ...
    protected void dropAllDeathLoot(net.minecraft.world.damagesource.DamageSource source) {
        ...
        PlayerLikeDeathDrop.drop(this, inventory, experienceLevel);
    }
    ...
    public void die(net.minecraft.world.damagesource.DamageSource source) {
        ...
        // Player-style inventory/equipment drops are not controlled by the
        // mob-loot gamerule. If the inherited Mob path skipped our override,
        // perform the same drop path here exactly once.
    ...
    public void addExperience(int amount) {
        ...
        while (totalExperience >= getXpAtLevel(level + 1)) level++;
