...
public final class BabyNPCPlayerItemRenderer
     ...
     */
    private static final Map<String, BabyItemRenderState> RENDER_STATES =
            new HashMap<>();
    ...
    /**
     * Keep the render-only proxy's equipment synchronized with the carried
     * ItemStack. The proxy is intentionally cached for GeckoLib animation
     * continuity, so equipment cannot be read only when the proxy is created.
     */
    private static void syncEquipmentFromData(
            BabyNPCPlayerEntity baby,
            CompoundTag data
    ) {
        if (!data.contains("BabyEquipment", CompoundTag.TAG_COMPOUND)) {
            return;
        }

        CompoundTag equipment = data.getCompound("BabyEquipment");
        syncEquipmentSlot(baby, equipment, "MainHand", net.minecraft.world.entity.EquipmentSlot.MAINHAND);
        syncEquipmentSlot(baby, equipment, "OffHand", net.minecraft.world.entity.EquipmentSlot.OFFHAND);
        syncEquipmentSlot(baby, equipment, "Head", net.minecraft.world.entity.EquipmentSlot.HEAD);
        syncEquipmentSlot(baby, equipment, "Chest", net.minecraft.world.entity.EquipmentSlot.CHEST);
        syncEquipmentSlot(baby, equipment, "Legs", net.minecraft.world.entity.EquipmentSlot.LEGS);
        syncEquipmentSlot(baby, equipment, "Feet", net.minecraft.world.entity.EquipmentSlot.FEET);
    }

    private static void syncEquipmentSlot(
            BabyNPCPlayerEntity baby,
            CompoundTag equipment,
            String key,
            net.minecraft.world.entity.EquipmentSlot slot
    ) {
        ItemStack stack = equipment.contains(key, CompoundTag.TAG_COMPOUND)
                ? ItemStack.of(equipment.getCompound(key))
                : ItemStack.EMPTY;

        ItemStack current = baby.getItemBySlot(slot);
        if (!ItemStack.matches(current, stack)) {
            baby.setItemSlot(slot, stack);
        }
    }
    ...
    public void renderByItem(
        ...
        String renderKey = babyUuid != null
                ? babyUuid.toString() + "|" + displayContext.name()
                : "fallback|" + displayContext.name();

        BabyItemRenderState renderState;
        if (babyUuid != null) {
            renderState = RENDER_STATES.get(renderKey);
            if (renderState == null || renderState.entity().isRemoved()) {
                ...
                RENDER_STATES.put(renderKey, renderState);
            }
        } else {
            if (fallbackRenderState == null
        ...
        // Equipment is part of the carried Baby's live appearance. The
        // render proxy is cached for animation continuity, so explicitly
        // synchronize its six equipment slots from the current ItemStack
        // snapshot before the entity renderer is invoked.
        syncEquipmentFromData(baby, data);

        // The proxy is deliberately not added to the client level, so it does
        // not receive Entity.tick(). GeckoLib still needs an entity clock for
        // its animation state. Use the real client-level game time here, not
        // render frames: game time advances at the same 20 ticks/sec cadence
        // as a Baby that is actually present in the world. This keeps idle1,
        // walk1, and every other animation at the exact same playback speed
        // in Item Mode and World Mode, regardless of FPS.
        long gameTime = minecraft.level.getGameTime();
        baby.tickCount = (int) (gameTime % 2_000_000_000L);
