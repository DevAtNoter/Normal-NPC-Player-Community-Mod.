...
public final class BabyDetailTooltipClientComponent implements ClientTooltipComponent {
    ...
    private static String identity(CompoundTag data) {
        String owner = data.contains("OwnerName") ? data.getString("OwnerName") : "";
        int health = data.contains("Health") ? Float.floatToIntBits(data.getFloat("Health")) : 20;
        float hunger = data.contains(BabyNPCPlayerEntity.FOOD_LEVEL_TAG) ? BabyNPCPlayerEntity.readFoodLevelTag(data, 20.0F) : data.contains("Hunger") ? data.getFloat("Hunger") : 20.0F;
        return owner + ':' + health + ':' + hunger + ':' + data.hashCode();
    }
