    ...
    public static void onPlayerDeath(LivingDeathEvent event) {
        ...
        for (var level : player.server.getAllLevels()) {
            for (Entity entity : level.getEntities().getAll()) {
                ...
                /*
                 * Life Bond belongs only to the biological parent relationship.
                 * Foster parents must NEVER participate in Life Bond: their
                 * death does not orphan/kill the child, and the child's death
                 * must not kill the foster parent.
                 */
                if (baby.isFoster()) {
                    continue;
                }
