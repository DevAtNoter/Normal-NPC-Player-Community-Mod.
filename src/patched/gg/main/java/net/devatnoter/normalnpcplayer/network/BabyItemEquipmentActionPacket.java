...
public final class BabyItemEquipmentActionPacket {
    ...
    private static void apply(BabyItemEquipmentActionPacket packet, ServerPlayer player) {
        ...
        // Armor can only be applied to its matching vanilla armor slot.
        // Non-armor items are valid only for the Baby's hands.
        if (carried.getItem() instanceof ArmorItem armor) {
            if (armor.getEquipmentSlot() != slot) return;
        } else if (slot != EquipmentSlot.MAINHAND && slot != EquipmentSlot.OFFHAND) {
            return;
        }

        // Main/offhand accept any normal item and preserve the full stack.
        // Armor remains a single equipped item.
        ...
        ItemStack incoming = hand ? carried.copy() : carried.copyWithCount(1);
        ItemStack cursorRemainder = carried.copy();
        cursorRemainder.shrink(incoming.getCount());
