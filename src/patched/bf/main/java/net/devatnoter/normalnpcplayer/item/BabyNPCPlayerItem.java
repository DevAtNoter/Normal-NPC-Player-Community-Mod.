...
public final class BabyNPCPlayerItem extends Item {
    ...
    private static final String CARRIED_VOICE_COOLDOWN_TAG = "BabyVoiceCooldown";
    private static final String CARRIED_NEXT_VOICE_TAG = "BabyNextVoiceTick";
    private static final int CARRIED_VOICE_COOLDOWN_TICKS = 60;
    private static final int CARRIED_IDLE_MIN_DELAY = 180;
    private static final int CARRIED_IDLE_MAX_DELAY = 360;
    private static final float CARRIED_HUNGRY_VOICE_CHANCE = 0.25F;
    ...
    public void inventoryTick(
        ...
        if (entity instanceof Player holder) {
            ...
            tickCarriedVocalization(stack, data, level, holder, now);
        } else {
    ...
    /**
     * Vocalization while the Baby is stored in the carried ItemStack. The
     * hunger and idle channels share one cooldown, so they cannot overlap.
     * The holder's movement counts as the Baby walking for ordinary chatter.
     */
    private static void tickCarriedVocalization(
            ItemStack stack,
            CompoundTag data,
            Level level,
            Player holder,
        ...
        if (!(level instanceof ServerLevel serverLevel) || holder == null) {
            return;
        }

        long cooldown = data.contains(CARRIED_VOICE_COOLDOWN_TAG)
                ? data.getLong(CARRIED_VOICE_COOLDOWN_TAG)
                : 0L;
        long nextVoice = data.contains(CARRIED_NEXT_VOICE_TAG)
                ? data.getLong(CARRIED_NEXT_VOICE_TAG)
                : now + 40L;

        if (now < cooldown || now < nextVoice) {
            return;
        }

        float hunger = BabyNPCPlayerEntity.readFoodLevelTag(data, 20.0F);
        boolean hungry = hunger < 10.0F
                || (hunger < 13.0F
                && serverLevel.random.nextFloat() < CARRIED_HUNGRY_VOICE_CHANCE);

        // A carried Baby is still a Baby. Idle chatter is not conditional on
        // the carrier moving; it must randomly vocalize while the holder is
        // standing still as well.
        Vec3 source = carriedBabyVoicePosition(holder);
        ...
        serverLevel.playSound(
                ...
                source.y,
                source.z,
                hungry ? ModSounds.BABY_HUNGRY.get() : ModSounds.BABY_IDLE.get(),
                SoundSource.NEUTRAL,
                0.85F,
        ...
        data.putLong(
                CARRIED_VOICE_COOLDOWN_TAG,
                now + CARRIED_VOICE_COOLDOWN_TICKS
        );
        data.putLong(
                CARRIED_NEXT_VOICE_TAG,
                now + CARRIED_IDLE_MIN_DELAY
                        + serverLevel.random.nextInt(
                                CARRIED_IDLE_MAX_DELAY - CARRIED_IDLE_MIN_DELAY + 1
                        )
        );
    }

    /**
     * Audio anchor for the Baby while it exists only as a carried ItemStack.
     * Do not use the player's eye position: that makes the Baby sound as if
     * it is inside the listener's head. The anchor is offset forward and
     * slightly downward, matching the carried Baby's visual space and giving
     * the client a real 3D source for direction and attenuation.
     */
    private static Vec3 carriedBabyVoicePosition(Player holder) {
        Vec3 eye = holder.getEyePosition(1.0F);
